...
class ArtworkDetailsAnnotationView : MKAnnotationView {
    ...
    private func setupImage(annotation: MKAnnotation?) {
        ...
        if let artworkDetailsAnnotation = annotation as? ArtworkDetailsAnnotation {
            if artworkDetailsAnnotation.artwork.isTargeted {
                image = #imageLiteral(resourceName: "Target Annotation Selected")
            }
            else if artworkDetailsAnnotation.artwork.isCollected {
