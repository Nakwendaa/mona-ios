...
//
//  Some modifications provided by Paul Chaffanet
...
open class TableViewIndex : UIControl {
    ...
    private func isProcessingTouch() {
        if let delegate = self.delegate
            , delegate.responds(to: #selector(TableViewIndexDelegate.tableViewIndexProcessingTouches(_:))) {
            
            delegate.tableViewIndexProcessingTouches!(self)
        }
    }
    
    private func isFinalizingTouch() {
        
        if let delegate = self.delegate
            , delegate.responds(to: #selector(TableViewIndexDelegate.tableViewIndexFinalizingTouches(_:))) {
            
            delegate.tableViewIndexFinalizingTouches!(self)
        }
    }
    
    /* End of three functions added by Paul Chaffanet */
    
    /* Modified by Paul Chaffanet */
...
public protocol TableViewIndexDelegate : NSObjectProtocol {
    ...
    @objc(tableViewIndexFinalizingTouches:)
    optional func tableViewIndexFinalizingTouches(_ tableViewIndex: TableViewIndex)
}
